...
// These #define values are specific to the BCM2835, taken from "BCM2835 ARM
// Peripherals"
...
void pioInit() {
  ...
  reg_map = mmap(
      ...
      PROT_READ |
          PROT_WRITE, // Enable both reading and writing to the mapped memory
      MAP_SHARED,     // This program does not have exclusive access to this
                      // memory
...
// pin: GPIO pin number to control (0-53)
// val: output value (1=HIGH/ON, 0=LOW/OFF)
...
void delayMicros(int micros) {
  ...
  while (SYS_TIMER_CSbits.M1 == 0)
    ; // wait until the match flag is set
}
...
void pwmInit() {
  ...
  // a "password" of 5A to the top bits to reduce the risk of accidental
  // writes.
...
void analogWrite(int val) { setPWM(78125, val / 255.0); }
